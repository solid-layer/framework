...
(cd cphalcon/ext; sudo export CFLAGS="-g3 -O1 -fno-delete-null-pointer-checks -Wall"; sudo phpize && sudo ./configure --enable-phalcon && sudo make -j4 && sudo make install && sudo echo "extension=phalcon.so" > phalcon.ini && sudo phpenv config-add phalcon.ini)
